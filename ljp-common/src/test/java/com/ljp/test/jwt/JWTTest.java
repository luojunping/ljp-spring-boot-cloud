...
public class JWTTest {
	...
	public static void main(String[] args) {
		...
		String token = JWT.create().withClaim("loginName", "SoYoung").withClaim("userName", "userName")
				.withClaim("email", "111111@qq.com").withExpiresAt(calendar.getTime())
				.sign(Algorithm.HMAC512("password"));
		System.out.println("token = " + token);
		DecodedJWT decode = JWT.decode(token);
		System.out.println("decode.getClaim(\"loginName\").asString() = " + decode.getClaim("loginName").asString());
		...
		// System.out.println(
		// "decodedJWT.getClaim(\"loginName\").asString() = " +
		// decodedJWT.getClaim("loginName").asString());
		// System.out.println("decodedJWT.getContentType() = " +
		// decodedJWT.getContentType());
		// System.out.println("decodedJWT.getExpiresAt() = "
		// + new SimpleDateFormat("yyyy-MM-dd
		// hh:mm:sss").format(decodedJWT.getExpiresAt()));
	}
